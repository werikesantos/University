...
public class Cliente {
   ...
   public ArrayList<Pedido> carregarPedidos(Connection conn, Date dataIni, Date dataFim) {
		String sqlSelect = "SELECT id, data, valor FROM pedido WHERE id_cliente = ? and "+
         "data between ? and ?";
		ArrayList<Pedido> lista = new ArrayList<>();
		
		try (PreparedStatement stm = conn.prepareStatement(sqlSelect);) {
			stm.setInt(1, getIdCliente());
         stm.setDate(2, new java.sql.Date(dataIni.getTime()));
